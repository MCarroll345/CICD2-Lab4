...
@app.put("/api/users/{user_id}", response_model=UserRead, status_code=status.HTTP_201_CREATED)
def update_user(user_id: int, payload: UserRead, db: Session = Depends(get_db)):
    ...
    user = UserDB(**payload.model_dump())
    try:
        stmt = update(UserDB).where(UserDB.id == user_id).values(id = user.id, name = user.name, email = user.email, age = user.age, student_id = user.student_id)
        db.execute(stmt)
        db.commit()
    except IntegrityError:
        db.rollback()
